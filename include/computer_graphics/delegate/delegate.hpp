...
#define DECLARE_DELEGATE(name, ...) using name = ::computer_graphics::delegate::Delegate<void, __VA_ARGS__>

#define DECLARE_DELEGATE_RETURN(name, return_type, ...) \
    using name = ::computer_graphics::delegate::Delegate<return_type, __VA_ARGS__>
...
constexpr DelegateBase::DelegateBase(std::nullptr_t) noexcept : handle_{nullptr} {}
...
class Delegate : public DelegateBase {
  ...
  public:
    ...
    template <typename T, typename... Payload>
    [[nodiscard]] static Delegate CreateSharedPtr(const std::shared_ptr<T> &object, SharedPtrFn<T, Payload...> function,
                                                  Payload... payload);
    template <typename T, typename... Payload>
    [[nodiscard]] static Delegate CreateSharedPtr(const std::shared_ptr<T> &object,
                                                  ConstSharedPtrFn<T, Payload...> function, Payload... payload);
...
Delegate<R, Args...> Delegate<R, Args...>::CreateStatic(StaticFn<Payload...> function, Payload... payload) {
    Delegate handler;
    handler.template Bind<StaticKind<Payload...>>(function, std::forward<Payload>(payload)...);
    return handler;
}
...
Delegate<R, Args...> Delegate<R, Args...>::CreateRaw(T *object, RawFn<T, Payload...> function, Payload... payload) {
    Delegate handler;
    handler.template Bind<RawKind<T, Payload...>>(object, function, std::forward<Payload>(payload)...);
    return handler;
}

template <typename R, typename... Args>
template <typename T, typename... Payload>
Delegate<R, Args...> Delegate<R, Args...>::CreateRaw(T *object, ConstRawFn<T, Payload...> function,
                                                     Payload... payload) {
    Delegate handler;
    handler.template Bind<ConstRawKind<T, Payload...>>(object, function, std::forward<Payload>(payload)...);
    return handler;
}
...
Delegate<R, Args...> Delegate<R, Args...>::CreateSharedPtr(const std::shared_ptr<T> &object,
    ...
    Delegate handler;
    handler.template Bind<SharedPtrKind<T, Payload...>>(object, function, std::forward<Payload>(payload)...);
    return handler;
}
...
Delegate<R, Args...> Delegate<R, Args...>::CreateSharedPtr(const std::shared_ptr<T> &object,
                                                           ...
                                                           Payload... payload) {
    Delegate handler;
    handler.template Bind<ConstSharedPtrKind<T, Payload...>>(object, function, std::forward<Payload>(payload)...);
    return handler;
}
...
Delegate<R, Args...> Delegate<R, Args...>::CreateLambda(Lambda &&lambda, Payload... payload) {
    Delegate handler;
    handler.template Bind<LambdaDelegate<Lambda, R(Args...), Payload...>>(std::forward<Lambda>(lambda),
...
template <typename R, typename... Args>
template <typename T, typename... Payload>
void Delegate<R, Args...>::BindSharedPtr(std::shared_ptr<T> object, SharedPtrFn<T, Payload...> function,
                                         Payload &&...payload) {
    static_assert(!std::is_const_v<T>, "Cannot bind a non-const function on a const object");
...
template <typename R, typename... Args>
template <typename T, typename... Payload>
void Delegate<R, Args...>::BindSharedPtr(std::shared_ptr<T> object, ConstSharedPtrFn<T, Payload...> function,
                                         Payload &&...payload) {
    *this = CreateSharedPtr<T, Payload...>(object, function, std::forward<Payload>(payload)...);
}
...
template <typename R, typename... Args>
R Delegate<R, Args...>::ExecuteIfBound(Args... args) const
requires(std::is_void_v<R>)
{
    if (!IsBound()) return;
