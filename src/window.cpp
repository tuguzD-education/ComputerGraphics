...
Window::Window(LPCTSTR name, LONG width, LONG height, HINSTANCE instanceHandle) {
    ...
    handle = CreateWindowEx(WS_EX_APPWINDOW, name, name, WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX | WS_THICKFRAME,
                            (GetSystemMetrics(SM_CXSCREEN) - width) / 2, (GetSystemMetrics(SM_CYSCREEN) - height) / 2,
                            rect.right - rect.left, rect.bottom - rect.top, nullptr, nullptr, instanceHandle, nullptr);
