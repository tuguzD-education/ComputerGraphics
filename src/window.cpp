...
Window::Window(
    std::string_view name, LONG width, LONG height, HINSTANCE instance_handle
  ...
  switch (u_message) {
    case WM_INPUT: {
      ...
      UINT result = ::GetRawInputData(
          h_raw_input, RID_INPUT, nullptr,
          &size, sizeof(RAWINPUTHEADER)
      );
      if (result) {
        throw std::runtime_error{
            "Failed to obtain raw input"
        };
      }

      lpb.reserve(size);
      result = ::GetRawInputData(
          h_raw_input, RID_INPUT, lpb.data(),
          &size, sizeof(RAWINPUTHEADER)
      );
      if (result != size) {
        throw std::runtime_error{
