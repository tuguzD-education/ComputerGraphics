#include "computer_graphics/debug_draw.hpp"
...
void DebugDraw::InitializePrimitiveVertexShader() {
    primitive_vertex_byte_code_ = detail::ShaderFromFile(
    "resources/shaders/debug_draw_primitive.hlsl", nullptr,
    D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_0",
    D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0);

    const HRESULT result = Device().CreateVertexShader(
        primitive_vertex_byte_code_->GetBufferPointer(),
        primitive_vertex_byte_code_->GetBufferSize(),
        nullptr, &primitive_vertex_shader_);
...
void DebugDraw::InitializePrimitivePixelShader() {
    primitive_pixel_byte_code_ = detail::ShaderFromFile(
        "resources/shaders/debug_draw_primitive.hlsl", nullptr,
        D3D_COMPILE_STANDARD_FILE_INCLUDE, "PSMain", "ps_5_0",
        D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0);
    const HRESULT result = Device().CreatePixelShader(
        primitive_pixel_byte_code_->GetBufferPointer(),
        primitive_pixel_byte_code_->GetBufferSize(),
        nullptr, &primitive_pixel_shader_);
...
void DebugDraw::InitializePrimitiveRasterizerState() {
    ...
    const HRESULT result = Device().CreateRasterizerState(
        &rasterizer_desc, &primitive_rasterizer_state_);
...
void DebugDraw::InitializePrimitiveVertexBuffer() {
    ...
    const HRESULT result = Device().CreateBuffer(
        &buffer_desc, nullptr, &primitive_vertex_buffer_);
...
void DebugDraw::UpdatePrimitiveVertexBuffer() {
    ...
    const HRESULT result = DeviceContext().Map(
        primitive_vertex_buffer_.Get(), 0,
...
void DebugDraw::DrawArrow(
    const math::Vector3& start, const math::Vector3& end, const math::Vector3& normal, const DrawOpts& opts) {
...
void DebugDraw::DrawEllipsoid(const Transform& transform, const EllipsisDrawOpts& opts) {
    ...
    const math::Quaternion xz_rotation =
        math::Quaternion::CreateFromYawPitchRoll(angle, 0.0f, 0.0f);
    ...
    const math::Quaternion yz_rotation =
        math::Quaternion::CreateFromYawPitchRoll(0.0f, angle, 0.0f);
...
void DebugDraw::Update(const float delta_time) {
    ...
    std::for_each(std::execution::par, primitive_vertices_.begin(),
        primitive_vertices_.end(), decrease_duration);
}
