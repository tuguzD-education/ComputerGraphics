...
TriangleComponent::TriangleComponent(
    Game &game, std::span<Vertex> vertices, std::span<Index> indices
) : Component{game} {
...
void TriangleComponent::Draw() {
  ...
  device_context->IASetIndexBuffer(
      index_buffer_.Get(), DXGI_FORMAT_R32_UINT, 0
  );
...
void TriangleComponent::InitializeVertexShader() {
  vertex_byte_code_ = detail::CompileFromFile(
      "resources/shaders/shader.hlsl",
...
void TriangleComponent::InitializeIndexShader() {
  ...
  index_byte_code_ = detail::CompileFromFile(
      "resources/shaders/shader.hlsl",
      shader_macros.data() /*macros*/, nullptr /*include*/,
      "PSMain", "ps_5_0",
      D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0
  );

  HRESULT result = GetDevice()->CreatePixelShader(
      index_byte_code_->GetBufferPointer(),
...
void TriangleComponent::InitializeRasterizerState() {
  ...
  HRESULT result = GetDevice()->CreateRasterizerState(
...
void TriangleComponent::InitializeVertexBuffer(std::span<Vertex> vertices) {
  ...
  HRESULT result = GetDevice()->CreateBuffer(
      &buffer_desc, &initial_data, &vertex_buffer_
  );
...
void TriangleComponent::InitializeIndexBuffer(std::span<Index> indices) {
  ...
  HRESULT result = GetDevice()->CreateBuffer(
      &buffer_desc, &initial_data, &index_buffer_
  );
