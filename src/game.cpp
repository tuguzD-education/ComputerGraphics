...
Game::Game(Window &window) : window_{window} {
  ...
  auto triangle_component =
      std::make_unique<TriangleComponent>(*this, vertices, indices);
...
void Game::Draw() {
  ...
  device_context_->ClearRenderTargetView(
      render_target_view_.Get(), &color.x
  );
