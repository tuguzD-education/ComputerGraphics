...
TriangleComponent::TriangleComponent(class Game &game, const Initializer &initializer)
    : SceneComponent(game, initializer),
      ...
      tile_count_{math::Vector2::One}, is_casting_shadow_{initializer.is_casting_shadow} {
...
void TriangleComponent::DrawInShadowMap(const Camera *camera) {
    ...
    if (!is_casting_shadow_ || vertex_buffer_ == nullptr || index_buffer_ == nullptr) return;
    ...
    device_context.VSSetShader(shadow_map_vertex_shader_.Get(),
        vs_class_instances.data(), vs_class_instances.size());
    ...
    device_context.VSSetConstantBuffers(0,
        vs_constant_buffers.size(), vs_constant_buffers.data());

    constexpr std::array<ID3D11ClassInstance *, 0> ps_class_instances{};
    device_context.PSSetShader(shadow_map_pixel_shader_.Get(),
        ps_class_instances.data(), ps_class_instances.size());
    ...
    device_context.IASetVertexBuffers(0, vertex_buffers.size(),
        vertex_buffers.data(), strides.data(), offsets.data());
...
void TriangleComponent::Draw(const Camera *camera) {
    if (vertex_buffer_ == nullptr || index_buffer_ == nullptr) return;
    ...
    constexpr std::array<ID3D11ClassInstance *, 0> vs_class_instances{};
    device_context.VSSetShader(vertex_shader_.Get(),
        vs_class_instances.data(), vs_class_instances.size());
    ...
    device_context.VSSetConstantBuffers(
        0, vs_constant_buffers.size(), vs_constant_buffers.data());

    constexpr std::array<ID3D11ClassInstance *, 0> ps_class_instances{};
    device_context.PSSetShader(pixel_shader_.Get(),
        ps_class_instances.data(), ps_class_instances.size());

    const PixelShaderConstantBuffer ps_constant_buffer{
        .has_texture = texture_ != nullptr,
        .view_position = (camera != nullptr) ?
            camera->WorldTransform().position : math::Vector3::Backward,
        .material = material_,
...
void TriangleComponent::InitializeVertexShader() {
    vertex_shader_byte_code_ = detail::ShaderFromFile(
        "resources/shaders/shader.hlsl", nullptr,
...
void TriangleComponent::InitializeShadowMapVertexShader() {
    shadow_map_vertex_shader_byte_code_ = detail::ShaderFromFile(
        "resources/shaders/shadow_map.hlsl", nullptr,
        D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_0",
        D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0);

    const HRESULT result = Device().CreateVertexShader(
        shadow_map_vertex_shader_byte_code_->GetBufferPointer(),
        shadow_map_vertex_shader_byte_code_->GetBufferSize(),
        nullptr, &shadow_map_vertex_shader_);
...
void TriangleComponent::InitializeShadowMapPixelShader() {
    ...
    const HRESULT result = Device().CreatePixelShader(
        shadow_map_pixel_shader_byte_code_->GetBufferPointer(),
        shadow_map_pixel_shader_byte_code_->GetBufferSize(),
        nullptr, &shadow_map_pixel_shader_);
...
void TriangleComponent::InitializeShadowMapRasterizerState() {
    ...
    const HRESULT result = Device().CreateRasterizerState(
        &shadow_map_rasterizer_desc, &shadow_map_rasterizer_state_);
    detail::CheckResult(result, "Failed to create shadow map rasterizer state");
}
...
void TriangleComponent::UpdateVertexShaderConstantBuffer(const VertexShaderConstantBuffer &data) {
    ...
    const HRESULT result = DeviceContext().Map(
        vertex_shader_constant_buffer_.Get(), 0,
...
void TriangleComponent::UpdatePixelShaderConstantBuffer(const PixelShaderConstantBuffer &data) {
    D3D11_MAPPED_SUBRESOURCE mapped_subresource{};
    const HRESULT result = DeviceContext().Map(
        pixel_shader_constant_buffer_.Get(), 0,
        D3D11_MAP_WRITE_DISCARD, 0, &mapped_subresource);
    detail::CheckResult(result, "Failed to map constant buffer data");
