...
void Game::OnInputKeyDown(const computer_graphics::InputKey input_key) {
    switch (input_key) {
        ...
        case computer_graphics::InputKey::G: {
            for (computer_graphics::Component &component : Components()) {
                if (const auto triangle_component = dynamic_cast<computer_graphics::TriangleComponent *>(&component)) {
                    triangle_component->Wireframe() = !triangle_component->Wireframe();
                }
                if (const auto primitive = dynamic_cast<computer_graphics::GeometricPrimitiveComponent *>(&component)) {
        ...
        case computer_graphics::InputKey::LeftButton: {
            ...
            for (const computer_graphics::Component &component : Components() | std::views::filter(is_scene_and_collision)) {
                const auto &scene_component = dynamic_cast<const computer_graphics::SceneComponent &>(component);
                const auto &collision_primitive = dynamic_cast<const computer_graphics::Collision &>(component);
