...
PlayerChild::PlayerChild(computer_graphics::Game& game, const Initializer& initializer)
...
void PlayerChild::ReInitializeVertexShader() {
    vertex_shader_byte_code_ = computer_graphics::detail::ShaderFromFile(
        "resources/shaders/katamari.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VSMain", "vs_5_0",
...
void PlayerChild::ReInitializeVertexShaderConstantBuffer() {
    ...
    computer_graphics::detail::CheckResult(result, "Failed to create vertex shader constant buffer");
}
...
void PlayerChild::UpdateVertexShaderConstantBuffer(const TriangleComponent::VertexShaderConstantBuffer& data) {
    ...
    computer_graphics::detail::CheckResult(result, "Failed to map constant buffer data");
